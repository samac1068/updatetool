...
export class PrimkeyDialogComponent implements OnInit {
  ...
  columnClickedHandler(col) {
    ...
    if(this.store.isInArray(this.selectCols, col.columnid))
      this.selectCols = this.selectCols.filter(x => x != col.columnid); // Item was in the array, so remove it
    else
  ...
  submitHandler() {
    ...
    for(let c = 0; c < this.selectCols.length; c++) {
        ...
        colnamestr+= this.availcol.find(x => x.columnid == this.selectCols[c]).columnname.toUpperCase();
    }
